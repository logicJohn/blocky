...
describe('Starting tests for blockyApp', function() {
  ...
  it('website title should be blockyApp', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('blockyApp');
    })
  });

  it('network-name should be blocky@1.0.0',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('blocky@1.0.0.bna');
    });
  });

  it('navbar-brand should be blockyApp',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('blockyApp');
    });
  });
